 ...
 *{box-sizing:border-box} body{margin:0;font-family:-apple-system,"Segoe UI",Roboto,"Malgun Gothic",sans-serif;background:var(--bg);color:var(--ink)}
 ...
 .gtitle{font-size:12px;font-weight:700;color:var(--sub);letter-spacing:.4px;margin-bottom:7px}
 .fields{display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end}
 .fields label{display:flex;flex-direction:column;gap:5px;font-size:12.5px;font-weight:600;color:var(--sub)}
 .fields input[type=number],.fields select{padding:8px 10px;border:1px solid var(--line);border-radius:8px;font-size:13.5px;background:var(--card);color:var(--ink);font-weight:400}
 .fields input[type=number]{width:110px}
 .fields label.chk{flex-direction:row;align-items:center;gap:6px;padding-bottom:9px}
...
<div class="wrap">
 <details class="match" id="matchbox" open>
  <summary>🎯 맞춤 검색 — 내 조건에 맞는 학교 찾기</summary>
  <div class="mgroup"><div class="gtitle">내 정보</div>
   <div class="fields">
  ...
  <div class="mgroup"><div class="gtitle">어학</div>
   <div class="fields">
    ...
    <label class="chk"><input type="checkbox" id="m_nolang"> 공인 어학성적 없음</label>
   </div>
  </div>
  <div class="mgroup"><div class="gtitle">지역</div>
   <div class="fields">
    <label>국가<select id="country"></select></label>
    <label>위치<select id="tier"></select></label>
   </div>
  </div>
  <div class="mgroup"><div class="gtitle">수업 언어</div>
   <div class="fields">
    <label>수업언어<select id="lang"></select></label>
   </div>
  </div>
  <div class="mactions">
  ...
  <div class="mnote">조건 미달 학교는 숨겨지고, 값이 "이메일 문의"·"학과별 상이" 등이라 판단할 수 없는 항목은 ⚠️로 표시됩니다(직접 확인 필요).
...
let M=null; // {g,s,mj,tier,t,i,none}
let F={c:'',lang:''}; // 적용 버튼으로 확정된 국가·수업언어 필터
...
// SKKU 공식 주관학부(대학) 목록 (킹고인포 기준, 2026-07) + 공과대학(사용자 추가)
const MAJORS=['경영대학','경제대학','공과대학','동아시아학술원','문과대학','법과대학','사범대학','사회과학대학',
...
function render(){
  ...
  let pairs=DATA.filter(r=>{
    if(F.c && r.country!==F.c) return false;
    if(F.lang && !val(r.lang_instruction).split(', ').includes(F.lang)) return false;
...
(function init(){
  ...
  $('#country').innerHTML='<option value="">전체</option>'+cs.map(c=>`<option>${c}</option>`).join('');
  ...
  $('#lang').innerHTML='<option value="">전체</option>'+ls.map(l=>`<option value="${l}">${l} (${lc[l]})</option>`).join('');
  const tc={};
  DATA.forEach(r=>{const t=(r.location||{}).tier; if(t) tc[t]=(tc[t]||0)+1;});
  $('#tier').innerHTML='<option value="">무관</option>'+['도시','준도시','도시 외곽'].filter(t=>tc[t]).map(t=>`<option value="${t}">${t} (${tc[t]})</option>`).join('');
